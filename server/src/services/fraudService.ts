...
function normalizeWallets(wallets: unknown): string[] {
...
export async function runFraudSweep(): Promise<void> {
  ...
  for (const [referrerId, referredIds] of byReferrer.entries()) {
    if (referredIds.length < 6) continue;

    const approvedDepositsCount = await Deposit.count({
      where: {
