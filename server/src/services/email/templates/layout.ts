...
export function renderEmailLayout(args: {
  ...
  const footer = args.footerNote
    ? `<p style="margin:0;color:#9CA3AF;font-size:12px;line-height:18px;">${escapeHtml(
        args.footerNote
      )}</p>`
  ...
  <tr>
    <td style="border-radius:999px;background:#C6A15B;">
      <a href="${escapeAttr(args.ctaHref)}" style="display:inline-block;padding:12px 18px;font-size:13px;font-weight:700;color:#0F0F10;text-decoration:none;border-radius:999px;">${escapeHtml(
        args.ctaLabel
      )}</a>
    </td>
  </tr>
                ...
                <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
                  <tr>
                    ...
                    <td align="right" style="vertical-align:middle;">
                      <a href="${escapeAttr(
                        baseUrl
                      )}" style="color:#9CA3AF;font-size:12px;text-decoration:none;">Dashboard</a>
                    </td>
                  </tr>
                </table>
              </td>
